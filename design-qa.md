...
The 17 browser checks also pass against the built CI preview (24.5 seconds). The preview enables its fixture entry explicitly; the normal production build excludes that entry. Fake-clock tests wait for each polling response between subsequent ticks. The visual runner dismisses the existing support-profile prompt before preparing authenticated routes so that overlay cannot intercept screenshot setup clicks.
