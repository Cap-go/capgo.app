...
import { DEFAULT_DEVICE_DATA_COLLECTION, parseDeviceDataCollection } from '~/services/deviceDataCollection'
...
export function useDeviceDataCollection(appId: MaybeRefOrGetter<string>) {
  ...
  async function load() {
    ...
    const { data } = await supabase
      ...
      .eq('app_id', id)
      .maybeSingle()
    collection.value = parseDeviceDataCollection(data?.device_data_collection)
  }
