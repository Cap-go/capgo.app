...
import { useObserveInsights } from '~/composables/useObserveInsights'
...
const { insights, insightsLoading, fetchInsights } = useObserveInsights({
  appId: id,
  rangeValue,
  selectedVersionName,
})
...
function openDeviceLogs(device: { action: string, device_id: string }) {
...
<template>
  <div>
    ...
    <div v-else-if="app" class="w-full h-full px-4 pt-0 mx-auto mb-8 sm:px-6 md:pt-8 lg:px-8 max-w-9xl max-h-fit">
      <div class="flex flex-col gap-6" data-test="observe-view-failure">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div class="min-w-0">
            <h2 class="text-lg font-semibold text-slate-900 dark:text-white">
              {{ t('observe-failure-heading') }}
            </h2>
            <p
