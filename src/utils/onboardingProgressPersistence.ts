...
interface CreateOnboardingProgressPersistenceOptions {
  ...
  write: (status: UserOnboardingStatus) => OnboardingPersistResult | Promise<OnboardingPersistResult>
}
...
export function createOnboardingProgressPersistence(options: CreateOnboardingProgressPersistenceOptions) {
  ...
  function persist(status: UserOnboardingStatus = 'in_progress') {
    ...
    chain = chain.then(async () => {
      ...
      try {
        const result = await options.write(status)
        if (result === 'conflict')
          blocked = true
        return result
      }
