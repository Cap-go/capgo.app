...
describe('app onboarding progress analytics integration', () => {
  ...
  it.concurrent('completes only terminal install or setup exits and leaves demo selection incomplete', () => {
    ...
    expect(cliRedirect).toContain(`if (flowStep.value === 'install' || flowStep.value === 'setup')`)
    expect(cliRedirect).toContain('progressTracker?.completeStep(flowStep.value, {')
    expect(cliRedirect).toContain('appId: createdApp.value.app_id')
    expect(cliRedirect).toContain('const maxAttempts = 3')
    expect(cliRedirect).toContain('const persistResult = await persistOnboardingProgress(\'completed\')')
    expect(cliRedirect).toContain('if (persistResult === \'persisted\')')
    expect(cliRedirect).toContain('if (persistResult !== \'retryable_failure\' || attempt === maxAttempts)')
    expect(cliRedirect).toContain('didRedirectToGettingStarted = false')
    expect(cliRedirect).toContain('router.replace')
    expect(cliRedirect.indexOf('completeStep')).toBeLessThan(cliRedirect.indexOf('persistOnboardingProgress'))
    expect(cliRedirect.indexOf('persistResult === \'persisted\'')).toBeLessThan(cliRedirect.indexOf('router.replace'))
