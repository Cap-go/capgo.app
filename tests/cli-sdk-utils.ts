...
async function getAuthorizedApp(
  ...
  if (!apiKey)
    return { error: 'Capgo authentication failed: invalid Capgo API key or insufficient Capgo permissions. This is not an App Store Connect / Google Play credential error.' as const }
...
export function createTestSDK(apikey: string = APIKEY_TEST_ORG_SUPER_ADMIN) {
  ...
  ;(sdk as any).updateChannel = async ({
    ...
    const apiKey = await getApiKeyRecord(apikey)
    if (!apiKey)
      return { success: false, error: 'Capgo authentication failed: invalid Capgo API key or insufficient Capgo permissions. This is not an App Store Connect / Google Play credential error.' }
