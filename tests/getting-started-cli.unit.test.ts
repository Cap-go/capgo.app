...
describe('getting started CLI setup panel', () => {
  ...
  it.concurrent('puts CLI, AI, and teammate setup on getting started instead of account onboarding', () => {
    ...
    expect(panelSource).toContain('text-pumpkin-orange-700')
    expect(panelSource).not.toContain('text-sky-300')
    expect(panelSource).not.toContain('text-violet-300')
    expect(panelSource).not.toContain('text-emerald-300')
    expect(panelSource).not.toContain('text-cyan-300')
