...
function rawVersion(properties: string) {
...
describe('buildFrontendOnboardingDailySetupCliHogql', () => {
  it('selects the bounded v2 and v3 Setup cohort and its Setup, copy, and unfiltered CLI events', () => {
    ...
    expect(setupPeople).toContain(`toIntOrZero(${rawVersion('properties')}) IN (2, 3, 4)`)
    expect(setupPeople).toContain(`${rawVersion('properties')} IN ('5.A', '5.C', '5.E', '5.F', '5.G')`)
    ...
    expect(selectedSetupCopyBranch).toContain(`toIntOrZero(${rawVersion('selected_events.properties')}) IN (2, 3, 4)`)
    expect(selectedSetupCopyBranch).toContain(`${rawVersion('selected_events.properties')} IN ('5.A', '5.C', '5.E', '5.F', '5.G')`)
