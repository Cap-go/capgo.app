...
describe('admin frontend onboarding dashboard', () => {
  ...
  it.concurrent('uses the existing admin dashboard components for v2 and the legacy funnel', async () => {
    ...
    const dailyCliOutcomeCard = source.match(/<ChartCard\s+\n?\s*:title="t\('frontend-onboarding-daily-setup-cli-outcomes-v2'\)"[\s\S]*?<\/ChartCard>/)?.[0] ?? ''
    expect(dailyCliOutcomeCard).toContain(':has-data="hasDailySetupCliOutcomeData"')
    expect(dailyCliOutcomeCard).toContain(`t('frontend-onboarding-daily-setup-cli-outcomes-description')`)
    expect(dailyCliOutcomeCard).toContain(':series="dailySetupCliSeries"')
    expect(dailyCliOutcomeCard).not.toContain(':total=')
    expect(dailyCliOutcomeCard).not.toContain(':unit=')
