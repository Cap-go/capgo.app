...
import { getStatsModeFromBody, isBillingOnlyStatsMode, sanitizeDeviceForLogging, sanitizeDeviceForPersistence, type StatsMode } from './stats_mode.ts'
...
function getStatsLogDimensions(c: Context, device: DeviceWithoutCreatedAt, statsMode: StatsMode = 'all'): StatsLogDimensions {
...
export async function onPremStats(c: Context, app_id: string, action: string, device: DeviceWithoutCreatedAt, metadata?: StatsMetadata, statsMode: StatsMode = 'all') {
  if (!app_id) {
  ...
  await createStatsLogsExternal(
    ...
    device.version_name,
    metadata,
    getStatsLogDimensions(c, device, statsMode),
  )
  const onPremLogContext = isBillingOnlyStatsMode(statsMode)
    ? sanitizeDeviceForLogging(device, statsMode)
    : { app_id: device.app_id, country: c.req.raw.cf?.country, user_agent: c.req.raw.headers.get('user-agent') }
  cloudlog({ requestId: c.get('requestId'), message: 'App is external (onPremise), returning 429', ...onPremLogContext })
...
export function createStatsDevices(c: Context, device: DeviceWithoutCreatedAt, statsMode: StatsMode = 'all') {
  const persistenceDevice = sanitizeDeviceForPersistence(device, statsMode)
  const requestCountry = c.req.raw?.cf?.country
  const countryCode = normalizeDeviceCountryCode(typeof requestCountry === 'string' ? requestCountry : undefined)
  const deviceWithCountry = !isBillingOnlyStatsMode(statsMode) && countryCode
    ? { ...persistenceDevice, country_code: countryCode }
    : persistenceDevice

  if (getRuntimeKey() === 'workerd' && c.env?.DEVICE_INFO)
    return backgroundTask(c, trackDevicesCF(c, deviceWithCountry, statsMode))
...
export function sendStatsAndDevice(c: Context, device: DeviceWithoutCreatedAt, statsActions: StatsActions[], isFailedStat = false, statsMode: StatsMode = 'all') {
  const dimensions = getStatsLogDimensions(c, device, statsMode)
  const jobs = []
  ...
  if (!isFailedStat)
    jobs.push(createStatsDevices(c, device, statsMode))
