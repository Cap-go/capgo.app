...
import { getStatsModeFromBody } from '../utils/stats_mode.ts'
...
async function post(c: Context, drizzleClient: ReturnType<typeof getDrizzleClient>, body: AppStats, appStatus?: AppStatusResult): Promise<PostResult> {
  ...
  const statsMode = getStatsModeFromBody(body)
  ...
  if (cachedStatus === 'onprem') {
    ...
    await onPremStats(c, app_id, action, device, metadata, statsMode)
  ...
  if (!appOwner.plan_valid) {
    ...
    await sendStatsAndDevice(c, device, upgradeActions, false, statsMode)
  ...
  if (!shouldRecordAction) {
    ...
    await sendStatsAndDevice(c, device, statsActions, action.endsWith('_fail'), statsMode)
    return { success: true }
  }
