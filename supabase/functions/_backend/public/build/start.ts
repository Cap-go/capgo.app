...
export async function startBuild(
  ...
  try {
    ...
    if (!(await checkPermission(c, 'app.build_native', { appId }))) {
      const errorMsg = 'Capgo permission denied: you do not have app.build_native to start builds for this app. This is Capgo authorization, not App Store Connect or Google Play credentials.'
      cloudlogErr({
        requestId: c.get('requestId'),
