    ...
    const onboardingMethodTrendQuery = sql`
      ...
      daily_methods AS (
        SELECT
          ...
          COUNT(*) FILTER (
            WHERE COALESCE(a.onboarding->>'source', 'manual') = 'manual'
          )::int as manual,
          COUNT(*) FILTER (
            WHERE a.onboarding->>'source' = 'cli'
          )::int as cli,
          COUNT(*) FILTER (
            WHERE a.onboarding->>'source' = 'mcp'
          )::int as mcp,
          COUNT(*) FILTER (
            WHERE a.onboarding->>'source' = 'ai'
          )::int as ai
        FROM public.apps a
    ...
    const onboardingOutcomeTrendQuery = sql`
      ...
      daily_outcomes AS (
        SELECT
          a.created_at::date as date,
          COUNT(*) FILTER (
            WHERE COALESCE(a.onboarding->>'outcome', 'in_progress') = 'completed'
          )::int as completed,
          COUNT(*) FILTER (
            WHERE a.onboarding->>'outcome' = 'skipped'
          )::int as skipped,
          COUNT(*) FILTER (
            WHERE a.onboarding->>'outcome' = 'switched_to_manual'
          )::int as switched_to_manual,
          COUNT(*) FILTER (
            WHERE COALESCE(a.onboarding->>'outcome', 'in_progress') = 'in_progress'
          )::int as in_progress
        FROM public.apps a
        WHERE a.created_at >= ${start_date}::timestamptz
          AND a.created_at < ${end_date}::timestamptz
          AND COALESCE(a.onboarding->>'source', 'manual') <> 'manual'
        GROUP BY a.created_at::date
      )
      SELECT
        ds.date,
