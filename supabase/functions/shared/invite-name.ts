...
export function containsDomainName(value: string) {
  const normalizedValue = value.replace(IDNA_DOT_PATTERN, '.')
  return Array.from(normalizedValue.matchAll(DOMAIN_CANDIDATE_PATTERN), ([candidate]) => candidate)
