...
ON public.build_requests USING btree (
    owner_org, builder_pool, status
);
...
CREATE TABLE IF NOT EXISTS public.dedicated_builders (
    ...
    FOREIGN KEY (requested_by) REFERENCES auth.users (
        id
    ) ON DELETE SET NULL,
...
'Org-scoped dedicated native builder requests and provisioning state. '
'Mutations go through the private API (service_role).';

COMMENT ON COLUMN public.dedicated_builders.status IS
'requested → provisioning → active (or suspended/cancelled). '
'Capgo ops advances status after provisioning the worker.';

COMMENT ON COLUMN public.dedicated_builders.allow_shared_fallback IS
'When true, builds may use the shared Capgo pool if the dedicated worker '
'is busy or offline.';

COMMENT ON COLUMN public.dedicated_builders.pool_id IS
'External builder pool identifier used when routing jobs to this org '
'dedicated worker.';
...
CREATE POLICY allow_service_role_full_access_to_dedicated_builders
