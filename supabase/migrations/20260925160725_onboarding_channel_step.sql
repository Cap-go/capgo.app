...
ADD CONSTRAINT "users_onboarding_valid" CHECK (
  ...
  AND (
    ...
    OR (
      ...
      AND (("onboarding" ->> 'step'::"text") = ANY (ARRAY['intent'::"text", 'publish_app_question'::"text", 'details'::"text", 'organization'::"text", 'choice'::"text", 'channel'::"text", 'install'::"text", 'setup'::"text"]))
    )
  )
  AND (
  ...
  AND (
    (NOT ("onboarding" ? 'setup_stage'::"text"))
    OR (
      ("jsonb_typeof"(("onboarding" -> 'setup_stage'::"text")) = 'string'::"text")
      AND (("onboarding" ->> 'setup_stage'::"text") = ANY (ARRAY['channel-routing'::"text", 'channel-self-assign'::"text", 'channel-console-assign'::"text", 'channel-create'::"text", 'cli'::"text"]))
    )
  )
) NOT VALID;
