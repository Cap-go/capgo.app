-- Make the r2_direct_manifest_jsonb guard message actionable for CLI users.
--
-- The guard in check_encrypted_bundle_on_insert (20260826101500,
-- 20260908120000) blocks app_versions.manifest jsonb writes on in-progress
-- r2-direct uploads, including the r2-direct -> r2 finalize. @capgo/cli older
-- than MIN_CLI_VERSION (8.42.3) finalizes that way, so its uploads hit this
-- exception after the files already reached R2. The CLI prints the raw
-- exception text, which named an internal endpoint (POST /private/set_manifest)
-- and gave the user no way to recover.
--
-- Only the raised message changes here: it now tells the user to update the
-- CLI, which is the real fix. The guard behavior, the pg_log reason
-- ('r2_direct_manifest_jsonb'), and every other branch stay identical to
-- 20260908120000.
