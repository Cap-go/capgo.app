...
CREATE OR REPLACE FUNCTION "public"."read_native_daily_platform_active"(
  "p_app_id" character varying,
  "p_period_start" timestamp without time zone,
  "p_period_end" timestamp without time zone
)
RETURNS TABLE("date" date, "platform" character varying, "devices" bigint)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
...
ALTER FUNCTION "public"."read_native_daily_platform_active"(
  "p_app_id" character varying,
  "p_period_start" timestamp without time zone,
  "p_period_end" timestamp without time zone
) OWNER TO "postgres";

COMMENT ON FUNCTION "public"."read_native_daily_platform_active"(
  "p_app_id" character varying,
  "p_period_start" timestamp without time zone,
  "p_period_end" timestamp without time zone
) IS 'Authorized distinct active native devices by calendar day and platform. Active means at least one device_usage report on that day.';

REVOKE ALL ON FUNCTION "public"."read_native_daily_platform_active"(
  "p_app_id" character varying,
  "p_period_start" timestamp without time zone,
  "p_period_end" timestamp without time zone
) FROM PUBLIC;

GRANT ALL ON FUNCTION "public"."read_native_daily_platform_active"(
  "p_app_id" character varying,
  "p_period_start" timestamp without time zone,
  "p_period_end" timestamp without time zone
) TO "service_role";
