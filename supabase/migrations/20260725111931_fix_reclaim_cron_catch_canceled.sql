...
SET
  target = 'public.cleanup_queue_messages()',
  description = 'Cleanup old queue messages (time-budgeted, no statement_timeout)',
  updated_at = now()
WHERE name = 'cleanup_queue_messages';

UPDATE public.cron_tasks
SET
  target = 'public.cleanup_old_audit_logs()',
  description = 'Delete audit_logs older than 30 days (time-budgeted batches)',
  updated_at = now()
WHERE name = 'cleanup_old_audit_logs';

UPDATE public.cron_tasks
SET
  target = 'public.null_migrated_app_version_manifests()',
  description = 'Null leftover app_versions.manifest arrays (time-budgeted, trigger bypass)',
  updated_at = now()
WHERE name = 'null_migrated_app_version_manifests';
