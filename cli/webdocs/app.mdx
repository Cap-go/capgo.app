...
When a new app is created, the CLI prints a console getting-started URL (`https://console.capgo.app/app/{appId}/getting-started`, or your configured `localWebHost` when set) so you can continue setup in the dashboard. Self-hosted Supabase deployments without `localWebHost` do not print a getting-started URL.
