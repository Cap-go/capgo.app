...
When you run an app command in an interactive terminal inside an identifiable Capacitor project, background checks can complete the integration-code and updater-installation onboarding tasks. If checks are still running when the command finishes, the CLI waits up to five seconds and shows a waiting message. It can exit sooner after the checks finish. Press Ctrl-C during the wait to exit immediately. CI, piped commands, and `app list --output-text` do not add a wait or message.
