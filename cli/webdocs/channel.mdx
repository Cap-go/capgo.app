...
Use a valid API key with `app.read` and `app.read_channels` permission for the app. Permission failures identify the required permission.
Settings display Yes/No, and channels without a bundle display Unlinked. Narrow terminals show each channel as a Setting/Value table.
