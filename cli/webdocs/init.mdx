...
Commands inside an identifiable Capacitor project can also complete Install Updater Plugin in the background when `@capgo/capacitor-updater` is declared in that app's package.json and installed locally. Hoisted and symlinked workspace installations are supported. This is best effort: commands may exit before detection finishes, and missing packages never clear an already completed step.
Background onboarding checks use the default Capgo API unless a custom API origin is explicitly trusted. Select your self-host with `--supa-host` and `--supa-anon`, or set `CAPGO_TRUSTED_API_ORIGINS` to a comma-separated list of trusted URL origins including scheme and port. Remote hosts require HTTPS; HTTP is allowed only for trusted loopback origins. Requests do not follow redirects.
