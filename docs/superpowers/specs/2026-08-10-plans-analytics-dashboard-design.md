...
> **Source of truth (2026-08-11):** Exact event tracking is the sole source of Plans openings. Only `User visit` events tagged with `page = 'plans'` contribute to the dashboard. Legacy pathname reconstruction is not shipped and is absent from both the response contract and the admin UI; the earlier reconstruction proposal is superseded.

## Summary

Add a read-only **Plans** page to the admin dashboard that explains how organizations use the customer-facing Plans page and whether those visits lead to checkout intent. The page combines exact PostHog Plans-opening events with Capgo billing records and makes incomplete historical billing classification visible instead of silently guessing.
...
4. Pure functions attribute checkout, reconstruct billing state, and produce every chart series.
...
An opening is a `User visit` event with `page = 'plans'`. Exact events are emitted once per Plans page activation by the tracking fix. Events without that page tag do not contribute to Plans traffic or checkout attribution.
...
The PostHog query therefore reads exact Plans visits from `start` through `end`, and checkout events from `start` through 24 hours after `end`. Checkout events outside the selected range support attribution only and never become visible Plans visits.
...
- **Total opens:** every exact Plans opening.
...
The failed 2026-08-10 PostHog pathname probes produced no trustworthy historical evidence. The shipped model does not expose reconstruction availability, a pathname failure reason, a legacy count, or a deduplication threshold; it simply counts exact page-tag events.
...
- Pure backend helpers own checkout attribution, paid-timeline reconstruction, and billing-category precedence.
...
- exact page-tag event filtering;
...
Before handoff, run focused tests, lint, type checking, and the production build.
...
- Exact page-tag event counts appear in data-quality metadata so tracking can be monitored.
