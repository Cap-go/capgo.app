...
- `onboarding_session_id`: the active PostHog browser session ID;
...
Expose a small read-only helper from the PostHog service for obtaining the
active session ID. Attach it as `onboarding_session_id` to the structured event
properties and to the Bento-forwarding request. Local development may return no
session ID because the existing PostHog integration is intentionally disabled
there; event delivery remains best-effort.
...
preference, and includes the verified organization, app, session ID, and safe
onboarding context. The backend verifies the caller's organization/app access
using the existing tracking-v2 flow before constructing the Bento payload.
