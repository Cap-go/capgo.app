# Capgo CLI AI Setup Prompt Draft

Status: Complete draft. Parts 1 through 8 are approved; conditional markers describe frontend-generated prompt branches.
...
## Part 1: Mandatory Capgo authentication

```text
You are helping me configure Capgo for my mobile app.
...
First, authenticate by appending:

login {API_KEY}

For example:

npx -y @capgo/cli@latest login {API_KEY}
...
## Part 2: Recommending the init command

```text
## 2. Choose guided init or AI-led setup
...
## Part 3: Organization and app selection

The conditional markers below describe how the frontend generates this section. They are not included in the prompt copied by the user.

````text
## 3. Select the Capgo organization and app
...
- Organization: {ORGANIZATION_NAME} (organization ID: `{ORGANIZATION_ID}`)
  ...
  There are {REMAINING_APP_COUNT} more applications available for this org. To list them, run the following command using the same ephemeral package runner selected in Section 1:
...
[IF one or more organizations were skipped while creating this API key]
...
## Part 4: Channel creation

```text
## 4. Create or select the update channel
...
## Part 5: Updater plugin installation

```text
## 5. Install the Capgo Updater plugin
...
## Part 6: notifyAppReady integration

````text
## 6. Add the notifyAppReady() startup call
...
## Part 7: Uploading the first bundle

```text
## 7. Upload the first bundle
...
## Part 8: Testing the first live update

````text
## 8. Test the first live update
